...
def construct_k2_repeated_map_saturation(eta: float, *, M: float = 1.0) -> K2SaturationResult:
    """Evaluate an exact k=2 saturation construction using one repeated law.

    Let ``P`` project onto ``e0`` and ``rho=eta*M``. Use the *same* bilinear
    law at the inner and root nodes:

    ``mu(x,y) = M*x1*y0*e0 + rho*x0*y0*e1``.

    Its operator norm is ``M`` for ``rho <= M`` and its projected-input
    closure defect is ``rho``. Unit leaves ``e0,e0,e0`` produce the ambient
    inner state ``rho*e1`` and projected inner state zero. The repeated law
    maps that normal state with coefficient ``M`` into ``e0``, so the
    projected-root error is exactly ``rho*M``.

    This closes fixed-eta sharpness for the declared real binary repeated-law
    class containing this map, rank-one orthogonal projection, and unit
    leaves. It does not assert sharpness for additional restrictions such as
    the earlier gated-planar rotation family.
    """
    ...
    def repeated(x: np.ndarray, y: np.ndarray) -> np.ndarray:
        return M * x[1] * y[0] * e0 + rho * x[0] * y[0] * e1

    inner_ambient = repeated(leaves[0], leaves[1])
    inner_projected = np.array([inner_ambient[0], 0.0])
    root_ambient = repeated(inner_ambient, leaves[2])
    root_projected = repeated(inner_projected, leaves[2])
    projected_error = float(np.linalg.norm(root_ambient - root_projected))
    bound = rho * M
    return K2SaturationResult(
        eta=eta,
