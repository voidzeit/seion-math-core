...
\begin{document}
\begin{abstract}
This companion documents the repository architecture, evidence contracts, test gates, paper-generation workflow, and governance state for the finite projected multilinear tree and graph program. The package separates executable behavior, mathematical claims, numerical artifacts, experiments, durable memory, and publication outputs. The v5 research layer adds an exact independent-law $k=2$ sharpness construction, the necessary-and-sufficient $k=2$ equality characterization, an exact contractive gated-planar repeated-law subclass, exact independent-law $k=3$ constants for every finite arity profile, the tagged same-law binary $k=3$ closure, the high-$\eta$ rank-one/common-leaf same-law chain closure, an exact unary-operator reduction for the remaining strict rank-one/common-leaf same-law chain, finite-tree asymptotic $(k-1)$ sharpness for arbitrary arity at least two, source-polynomial DAG propagation, signed-expression certificates, validated finite norm enclosures, and approximate-law budgets. The current verification collects 493 tests, with the mathematical closure and non-slow KGR segments passing; the FB15K237 full batched training epoch completes but remains above the preregistered 300-second performance ceiling on this hardware. Governance passes structurally with known historical warnings. The package remains fail-closed for theorem novelty, independent human review, the unresolved low-$\eta$ operator extremum and gated variants, fixed-eta independent-law sharpness for $k\ge4$, and growing-tree uniformity.\end{abstract}
...
\texttt{b817624} & M8/M9 closure & $k=2$ equality characterization and unconditional $k=3$ envelope\\
\texttt{9c8e755} & M10 correction & non-attainment only; strict supremum gap re-opened\\
\bottomrule
\end{tabularx}
\end{center}
...
The latest structural audit snapshot reports 156 historical runs, 9 unique
scientific instances, 8 duplicate groups, 0 missing artifacts, and complete
contracts for all 156 records. The duplicate warning is a governance issue,
not a reason to reinterpret historical runs as independent evidence.
...
theorem-level novelty, independent human review, variable-gate/arbitrary-leaf
gated variants beyond the contractive M17 class,
arbitrary-tree $k=3$ sharpness, universal dimension/rank reduction beyond
fixed trees, and global multilinear norm optimality are unresolved. The
independent-law binary $k=3$ chain and branching constants are closed by M14
and M15.
