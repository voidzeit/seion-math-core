"""Hostile edge-case checks for the Theorem R proof (review package v1), real field.
...
for trial in range(6000):
    ...
    for i in range(m):
        ...
        kids.append(child_pair(rho, psi, str(rng.choice(["id", "zero", "rank1", "orth"]))))
...
summary = {"cases": len(rows), "max_v_O2": max(r["v_O2"] for r in rows), "max_v_root": max(r["v_root"] for r in rows),
           ...
           "counts_eta1": sum(r["eta"] == 1.0 for r in rows), "counts_rho_zero": sum(r["rho_in"] == 0 for r in rows),
           "counts_chi_ge_pi": sum(r["chi_in"] >= math.pi for r in rows), "worst": worst}
json.dump(summary, open("outputs/edge_cases.json", "w"), indent=1)
