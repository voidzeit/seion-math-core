        ...
        for path in paths:
            if path.is_file():
                ...
                for child in path.rglob("*"):
                    ...
                    and not any(part in {"runtime", "machine", "packs"} and ".ai" in child.parts for part in child.parts)
...
def main() -> int:
    ...
    companion_pdfs = {
        ROOT / "papers" / "foundations" / "build" / "main.pdf": "seion_math_core_research_paper.pdf",
        ROOT / "papers" / "software" / "build" / "main.pdf": "seion_math_core_software_companion.pdf",
    }
    for companion_pdf, destination_name in companion_pdfs.items():
        if companion_pdf.exists():
            shutil.copy2(companion_pdf, RELEASE / destination_name)
    source_paths = [
        ROOT / "src",
        ROOT / "tests",
        ROOT / "experiments",
    ...
    (RELEASE / "REPRODUCE.md").write_text("# Reproduce\n\n```powershell\npython -m pip install -e .\n.\\scripts\\run_full_blackwell.ps1\n.\\scripts\\build_all_artifacts.ps1\n.\\scripts\\build_paper.ps1\n.\\scripts\\build_companions.ps1\n```\n\nThe strict governance audit is the release predicate. The paper command is `.\\scripts\\build_paper.ps1`; the independent research and software manuscripts are built by `.\\scripts\\build_companions.ps1`.\n", encoding="utf-8")
    entries = []
